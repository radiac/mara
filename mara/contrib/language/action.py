...
from .dictionary import VERBS, SOCIAL_PREPOSITIONS
                ...
                if word in VERBS:
                    ...
                    if user:
                        ...
                        if (
                            ...
                            last_word_token and
                            isinstance(last_word_token, VerbToken) and
                            # Last word should have had a preposition here
                            last_word_token.value in SOCIAL_PREPOSITIONS and
                            # Last word was first, just forgot preposition
                            not last_word_token.last_word_token
                        ):
                            # Add it back
                            self.tokens.append(
                                OtherToken(SOCIAL_PREPOSITIONS[
                                           token.value] + ' ')
                            )
        ...
        for token in self.tokens:
            ...
            # Change first person pronouns (me) to second person (you)
                ...
                # the user
                    ...
                    if (
                        prn_gender in last_user and
                        last_user[prn_gender] == user
                    ):
    ...
    def third_person(self, target=None, source=None):
        ...
        for token in self.tokens:
            ...
            # Change username
                ...
                token_value = token.value.lower()
                # First person pronouns to third person using source gender
                if token_value.upper() == pronouns.first.subject:
                    # I to he/they
                    word = source_gender.subject
                elif token_value == pronouns.first.object:
                    # me to him/they
                    word = source_gender.object
                elif token_value == pronouns.first.possessive:
                    # my to his/their
                    word = source_gender.possessive
                elif token_value == pronouns.first.self:
                    # myself to himself/themselves
                    word = source_gender.self
