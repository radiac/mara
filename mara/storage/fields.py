...
class Field(object):
    ...
    def serialise(self, obj, name, session):
        """
        ...
        return self.serialise_value(value, session)

    def get_value(self, obj, name):
        return getattr(obj, name)

    def serialise_value(self, data, session):
        """
        ...
        * Stores key references to Store and SessionStore objects
        ...
        if isinstance(data, dict):
            return {
                self.serialise_value(key): self.serialise_value(value, session)
                for key, value in data.items()
            }

        elif isinstance(data, list):
            # No duck typing here - we need to deserialise exactly
            return [self.serialise_value(value, session) for value in data]
    ...
    def deserialise(self, obj, name, data, session):
        ...
        """
        value = self.deserialise_value(obj, data, session)
        self.set_value(obj, name, value)

    def set_value(self, obj, name, data):
        setattr(obj, name, data)

    def deserialise_value(self, obj, data, session):
        """
        ...
        if isinstance(data, dict):
            ...
            return {
                self.deserialise_value(obj, key, session):
                self.deserialise_value(obj, value, session)
                for key, value in data.items()
            }
        elif isinstance(data, list):
            return [
                self.deserialise_value(obj, value, session) for value in data
            ]
